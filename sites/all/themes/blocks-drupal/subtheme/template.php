 ...
 */
function redcrackle_preprocess_username(&$vars) {
  global $theme_key;
 ...
 */  
function redcrackle_preprocess_page(&$vars, $hook) {
  if (isset($vars['node'])) {
...
function redcrackle_process_page(&$variables) {	
  ...
  if( !empty($tid) && arg(1) == 'term'){  
	...
	taxonomy_term_load($tid);
        ...
        if (!empty($uri)) {
	    $variables['image_url'] = theme('image_style', array(
                'path' => $uri,      
                'style_name' => 'large',
                'alt' => $alt,
            ));
 ...
 */
function redcrackle_menu_link(array $variables) {
 ...
 */
function redcrackle_links($variables) {
 ...
 */
function redcrackle_field($variables) {
 ...
 */
function redcrackle_breadcrumb($variables) {
  $count = '100';
 ...
 */
function redcrackle_preprocess_html(&$vars){
  global $root;
